    ...
    match options.subcommand {
        ...
        Subcommand::Deploy(options) => {
            ...
            println!();
            {
                ...
                ssh_channel.exec(
                    format!("DEBIAN_FRONTEND=noninteractive sudo apt-get -y --fix-broken install -o DPkg::options::=\"--force-confdef\" -o DPkg::options::=\"--force-confold\" {}", remote_package_file_path).as_str(),
                )?;
